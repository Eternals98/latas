...
export const DashboardScreen = ({ go }: { go: (id: string) => void }) => {
  ...
  return (
    <div className="p-8 max-w-[1400px] mx-auto animate-in fade-in duration-500">
      ...
      <Card className="mt-5 p-6 md:p-7 grid grid-cols-[1fr_240px_240px] gap-8 items-center hover:shadow-2 transition-shadow">
        <div>
...
const HeroFigure = ({ label, value, delta, deltaTone = "sage", note, accent = false }: { label: string, value: number, delta?: string, deltaTone?: string, note?: string, accent?: boolean }) => (
  <div className={cn(
    "flex flex-col gap-2.5 min-h-[150px] p-5.5 rounded-xl border transition-all hover:shadow-2",
...
const QuickAction = ({ icon, label, onClick, accent }: { icon: string, label: string, onClick: () => void, accent?: boolean }) => (
  <button
    onClick={onClick}
    className={cn(
      "flex items-center gap-2.5 p-3.5 rounded-lg border font-sans font-semibold text-sm transition-all hover:shadow-2 w-full text-ink-900",
