...
export const SaleScreen = ({
  cashOpen, openCash, companies, customers, paymentMethods,
  defaultCompanyId, submitting, error, onSubmit, onCancel,
}: SaleScreenProps) => {
        ...
        <div className="p-8">
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-[1.2fr_1fr] gap-6">
            <div className="flex flex-col gap-2">
              <label htmlFor="customer-select" className="text-[11px] tracking-overline uppercase font-semibold text-ink-900">Cliente / Entidad</label>
              <div className="relative">
                <span className="absolute top-3 left-3.5 text-slate-400" aria-hidden="true"><Icon name="users" size={16}/></span>
                <select
                  id="customer-select"
                  className="w-full px-3.5 py-2.5 pl-11 pr-10 rounded-sm border border-slate-200 bg-white outline-none focus:border-brass-400 focus:ring-2 focus:ring-brass-400/25 focus-visible:ring-2 focus-visible:ring-brass-400/50 transition-all text-[15px] font-sans appearance-none"
                  value={customerId}
              ...
              {customerId && customers.find(c => c.id === customerId)?.document && (
                <span className="text-[11px] text-slate-600 font-mono">ID {customers.find(c => c.id === customerId)?.document}</span>
              )}
              <div className="text-xs text-slate-500">Seleccione de clientes registrados</div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              <label htmlFor="transaction-date" className="text-[11px] tracking-overline uppercase font-semibold text-ink-900">Fecha del movimiento</label>
              <div className="relative">
            ...
            <div className="md:col-span-2 flex flex-col gap-2">
              <label htmlFor="description" className="text-[11px] tracking-overline uppercase font-semibold text-ink-900">Descripción de la venta</label>
              <textarea
                id="description"
          ...
          {/* Error banner */}
          {error && (
            <div className="flex items-center gap-3 bg-coral-50 border border-coral-500/30 text-coral-700 p-4 rounded-xl mt-6 animate-in fade-in slide-in-from-top-2">
      ...
      <aside className="flex flex-col gap-4 lg:sticky lg:top-6 self-start">
        ...
        <div className="mt-2 flex flex-col gap-3">
          <Button
            ...
            className="h-14 text-base shadow-2"
            onClick={handleSubmit}
            icon={submitting ? 'spinner' : 'check'}
          >
            {submitting ? 'Registrando…' : 'Registrar transacción'}
          </Button>
          <Button variant="ghost" size="lg" className="h-13" onClick={resetForm}>
            Cancelar y limpiar
          </Button>
        </div>
...
const SummaryCard = ({ label, value, variant = "default", tone }: { label: string, value: number, variant?: 'default' | 'inverse', tone?: 'success' | 'danger' | 'neutral' }) => {
  ...
  return (
    <Card
      variant={variant}
      className={cn(
        "p-5 px-6 transition-all hover:shadow-2",
